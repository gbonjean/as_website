...
class ImageService {
  ...
  ImageService() {
    photos = _db.collection('medias').orderBy('created_at', descending: true).snapshots().map((snapshot) => snapshot
        .docs
        .map(
            (doc) => Photo.fromJson(doc.id, doc.data()))
        .toList());
  }
  ...
  Future<void> addPhoto(Uint8List fileBytes, String fileName) async {
    ...
    if (rawImage != null) {
      ...
      final storageRef = _storage.ref('photos/${now.year}/${now.month}/$name-${image.width}x${image.height}.jpg');
